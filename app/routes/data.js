            ...
            for (j = 0; j < arrayDiscussions.length; j++) {
                ...
                request(optionsDiscussions, function(error, response, body) {
                    ...
                    if ((error_requests+completed_requests) == arrayDiscussions.length) {
						...
						console.log("Completed requests: "+completed_requests+" out of "+arrayDiscussions.length+". Error requests: "+error_requests);
    ...
    request(options, function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            for (j = 0; j < arrayDiscussions.length; j++) {
				...
				console.log("Analizando discusión "+arrayDiscussions[j]['discussionid']);
                request(optionsDiscussions, function(error, response, body) {
                    if (!error && response.statusCode == 200) {
                        completed_requests++;
                        var respuesta = JSON.parse(body);
